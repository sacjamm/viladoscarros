    ...
    public function update(Request $request)
    {
       /* if(env('PROJECT_MODE') == 0) {
        ...
        if ($request->hasFile('login_page_banner')) {
            ...
            $currentLoginBannerPath = public_path('uploads/page_banners/' . $request->current_login_page_banner);
            if (file_exists($currentLoginBannerPath)) {
                unlink($currentLoginBannerPath);
            } else {
        ...
        if ($request->hasFile('registration_page_banner')) {
            ...
            $currentRegisterBannerPath = public_path('uploads/page_banners/' . $request->current_registration_page_banner);
            if (file_exists($currentRegisterBannerPath)) {
                unlink($currentRegisterBannerPath);
            } else {
                // O arquivo não existe, você pode logar uma mensagem ou tratar o erro conforme necessário
                //Log::warning('O arquivo não foi encontrado: ' . $currentFaviconPath);
            }
        ...
        if ($request->hasFile('forget_password_page_banner')) {
            ...
            $currentForgetBannerPath = public_path('uploads/page_banners/' . $request->current_forget_password_page_banner);
            if (file_exists($currentForgetBannerPath)) {
                unlink($currentForgetBannerPath);
            } else {
                // O arquivo não existe, você pode logar uma mensagem ou tratar o erro conforme necessário
                //Log::warning('O arquivo não foi encontrado: ' . $currentFaviconPath);
            }
        ...
        if ($request->hasFile('customer_panel_page_banner')) {
            ...
            $currentCustomerBannerPath = public_path('uploads/page_banners/' . $request->current_customer_panel_page_banner);
            if (file_exists($currentCustomerBannerPath)) {
                unlink($currentCustomerBannerPath);
            } else {
                // O arquivo não existe, você pode logar uma mensagem ou tratar o erro conforme necessário
                //Log::warning('O arquivo não foi encontrado: ' . $currentFaviconPath);
            }
