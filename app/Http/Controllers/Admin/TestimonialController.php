...
class TestimonialController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
       ...
       /* if(env('PROJECT_MODE') == 0) {
        ...
        if ($request->hasFile('photo')) {
            ...
            $currentTestimonialsPath = public_path('uploads/testimonials/' . $request->current_photo);
            if (file_exists($currentTestimonialsPath)) {
                unlink($currentTestimonialsPath);
            } else {
    ...
    public function destroy($id)
    {
       ...
       /* if(env('PROJECT_MODE') == 0) {
        ...
        $testimonial = Testimonial::findOrFail($id);
        $currentTestimonialsPath = public_path('uploads/testimonials/' . $testimonial->photo);
            if (file_exists($currentTestimonialsPath)) {
                unlink($currentTestimonialsPath);
            } else {
                // O arquivo não existe, você pode logar uma mensagem ou tratar o erro conforme necessário
