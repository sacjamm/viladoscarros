...
class SettingController extends Controller {
    ...
    public function update(Request $request) {
        ...
        if ($request->logo != '') {
            ...
            $currentLogoPath = public_path('uploads/site_photos/' . $request->current_logo);
            if (file_exists($currentLogoPath)) {
                ...
                //Log::warning('O arquivo não foi encontrado: ' . $currentLogoPath);
            }
        ...
        if ($request->favicon != '') {
            ...
            $currentFaviconPath = public_path('uploads/site_photos/' . $request->current_favicon);
            if (file_exists($currentFaviconPath)) {
                unlink($currentFaviconPath);
            } else {
                // O arquivo não existe, você pode logar uma mensagem ou tratar o erro conforme necessário
        ...
        $data['facebook_access_token'] = $request->input('facebook_access_token');
        $data['facebook_catalog_id'] = $request->input('facebook_catalog_id');

        $data['c2s_token'] = $request->input('c2s_token');
        ...
        $data['google_maps_api_key'] = $request->input('google_maps_api_key');
        $data['google_maps_status'] = $request->input('google_maps_status');
        
        
        $data['google_tag_manager_body'] = $request->input('google_tag_manager_body');
        $data['google_tag_manager_noscript'] = $request->input('google_tag_manager_noscript');
        $data['google_tag_manager_status'] = $request->input('google_tag_manager_status');
